...
// import { useAllStockProductsQuery, useConfirmStockMutation, useCreateStockMutation, useStockDataQuery, useCancelStockMutation, useSearchProductQuery, useDeleteBatchMutation } from "@/redux/feature/stockSlice";
...
//     //   const res = await deleteBatch({id: product.batch_id}).unwrap();
//       const res = await deleteBatch({ id: stockDatas.batch_id }).unwrap();

//     //   console.log("Product removed successfully:", product.batch_id);
...
export default function Stock() {
  ...
  const handleAddStock = async () => {
    if (!selectedProduct || !stockQuantity || !costPrice || !sellingPrice || !mrp) {
      toast.error("Please fill in all fields.");
      return;
    }
    if (isNaN(Number(stockQuantity)) || isNaN(Number(costPrice)) || isNaN(Number(sellingPrice)) || isNaN(Number(mrp))) {
  ...
  const handleRemoveProduct = async (item) => {
    if (!stockDatas?.batch_id) {
      toast.error("No valid batch ID available to delete.");
      return;
    }
    ...
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Do you want to delete the entire batch (Batch ID: ${stockDatas.batch_id})? This will remove all products in the batch.`,
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#d33",
        cancelButtonColor: "#3085d6",
      ...
      if (result.isConfirmed) {
        const res = await deleteBatch(stockDatas.batch_id).unwrap();
      ...
      const errorMessage = error?.data?.message || "Failed to delete batch. Please try again.";
